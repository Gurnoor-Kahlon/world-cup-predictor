...
> The images below are **labeled placeholders**. Replace them with real screenshots
> (instructions follow) — keep the same file names and the README updates itself.

| Match Predictor | Team Comparison | Scoreline Heatmap |
|---|---|---|
| ![Match Predictor](app/assets/screenshots/match_predictor.png) | ![Team Comparison](app/assets/screenshots/team_comparison.png) | ![Scoreline Heatmap](app/assets/screenshots/scoreline_heatmap.png) |

| Tournament Simulator | Backtesting Dashboard | 2026 World Cup Mode |
|---|---|---|
| ![Tournament Simulator](app/assets/screenshots/tournament_simulator.png) | ![Backtesting Dashboard](app/assets/screenshots/backtesting_dashboard.png) | ![2026 World Cup Mode](app/assets/screenshots/worldcup_2026_mode.png) |

<details>
<summary><b>📸 How to add real screenshots</b></summary>

1. Run the app: `streamlit run app/streamlit_app.py`
2. Open each page from the left navigation and screenshot the main panel.
3. Save each image into `app/assets/screenshots/`, **overwriting** the placeholder
   with the matching name:

   | Page | File to overwrite |
   |------|-------------------|
   | Match Predictor | `match_predictor.png` |
   | Team Comparison | `team_comparison.png` |
   | Scoreline Heatmap | `scoreline_heatmap.png` |
   | Tournament Simulator | `tournament_simulator.png` |
   | Backtesting Dashboard | `backtesting_dashboard.png` |
   | 2026 World Cup Mode | `worldcup_2026_mode.png` |

4. (Optional) Record a short **demo GIF** with [ScreenToGif](https://www.screentogif.com/)
   (Windows) or [LICEcap](https://www.cockos.com/licecap/), save it as
   `app/assets/screenshots/demo.gif`, and add `![demo](app/assets/screenshots/demo.gif)`
   near the top of this section.

The placeholders were generated by `python app/assets/screenshots/make_placeholders.py`.
</details>
...
> New to Python or hit a snag? See **[QUICKSTART.md](QUICKSTART.md)** for
> step-by-step Windows/macOS/Linux instructions and troubleshooting.
...
only**; connect real data for meaningful figures.

**What the metrics mean (plain English):**

- **Accuracy** — how often the single most likely outcome was right (ignores confidence).
- **Log loss** — rewards probabilities that are correct *and* well-calibrated; punishes
  confident mistakes. **Lower is better**; below `1.099` beats blind guessing.
- **Brier score** — average squared error of the probability vector. **Lower is better**
  (0 = perfect).
- **Calibration** — when the model says "60%", does it happen ~60% of the time?

Run it yourself (results are also explorable on the app's **Backtesting** page):

```bash
python scripts/run_backtest.py                       # default 25% holdout
python scripts/run_backtest.py --test-fraction 0.3
python scripts/run_backtest.py --tournament "World Cup"
```

Outputs are saved to `data/processed/backtest_results.csv` (per-match predictions) and
`data/processed/backtest_comparison.csv` (the table above). Full method and limitations:
